...
**v0.3.9 / build 15：开发预览。** 临时音频检测故障按 3/10/30 秒有限自动重建，不再全部锁死等待手工核对；音量写入失败仍安全锁定。新增本机最近 64 条诊断记录，保留手动核对前的故障。恢复音量仍须用户确认。实机与长期性能限制见[测试报告](docs/TESTING.md)。

恢复音量选择预览（原生视图、模拟设备数据）：

![恢复音量滑块](docs/assets/v0.3.8/recovery-prompt-dark.png)

页首为 v0.2.0 实机截图。以下为 v0.3.4 的历史原生恢复提示预览（模拟数据）：
...
- 默认倒计时 60 秒，可设 5 秒至 10 分钟，拖动音量滑块时暂停。滑动不出声，点击“恢复到 …%”才生效；“保持静音，不再提醒”结束本次恢复监测，左上角关闭或超时只是临时收起，下次播放仍可提示。
...
- [v0.3.9 自动恢复与故障诊断](docs/prd/v0.3.9/prd.md)
- [v0.3.8 恢复音量选择](docs/prd/v0.3.8/prd.md)
- [v0.3.7 重新核对保留恢复记录](docs/prd/v0.3.7/prd.md)
