...
**v0.3.5 / build 10：开发预览。** “重新核对”现在先释放 Core Audio 检测，冷却 3 秒后只重建一次，避免严格静音流检测因资源竞争进入锁存故障并停止自动归零。恢复提醒仍默认关闭。尚未完成全部设备、辅助功能权限和长期性能实机验收，具体证据及限制见[测试报告](docs/TESTING.md)。
