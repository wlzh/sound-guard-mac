...
Universal 构建分别指定 arm64/x86_64 triple 后 lipo 合并，避免本机精简工具链缺失 xcbuild。发行包 ad-hoc 签名，无 Developer ID/公证。发布到公开 GitHub 仓库，预览 Release 附 ZIP 与 SHA256，不把 Git tag 当作资产验证。
...
Visuals 管理原生样式和 SVG 资源；MenuUI、SettingsUI、AboutUI 拆分展示。GuardPresentation 提供纯状态文本。UIChecks 使用模拟设备，不启动监听或持久化偏好。无新后台计时器，菜单关闭释放自定义视图，窗口关闭释放引用。
