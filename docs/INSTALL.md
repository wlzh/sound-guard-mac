...
v0.3.5 / build 10，开发预览；macOS 14.2+，Universal arm64/x86_64。当前已验证的硬件结论仍来自旧版 Apple Silicon 测试，其他平台及新增功能需实机验证。

## 安装

从 [公开 Release](https://github.com/wlzh/sound-guard-mac/releases) 下载 `SoundGuard-v0.3.5-macos-universal.zip` 和 SHA256.txt，在同一目录执行 `shasum -a 256 -c SHA256.txt`。解压后把 Sound Guard.app 放入 `/Applications` 或 `~/Applications` 再打开。
