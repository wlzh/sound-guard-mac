...
func runUIChecks(outputDirectory: URL?) throws {
    ...
    let preview = NativeSurface(frame: NSRect(x: 0, y: 0, width: 316, height: 440))
    let rows = NSStackView(); rows.orientation = .vertical; rows.alignment = .leading; rows.spacing = 2
    for entry in menu.items {
        if entry.view != nil {
            let view = UI.menuHeader(headline: "音量已归零", detail: "内建扬声器 · 0%")
    ...
    print("UI_CHECKS=PASS; ASSERTIONS=12; PREVIEWS=SYNTHETIC; MENU_PREVIEW=STRUCTURE_NOT_OS_SCREENSHOT")
}
